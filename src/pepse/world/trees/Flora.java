...
public class Flora extends Scrollable<Tree> {
	...
	private static final int ODD_CONST = 0x45d9f3b;
	private final int seed;
	private final GroundHeightAt groundHeightAt;
	private static final int NON_NEGATIVE = 0x7FFFFFFF;
	...
	public float pseudoRandomFloatAt(int x) {
		int z = x ^ seed;
		z = (z ^ (z >>> SIXTEEN)) * ODD_CONST;
		z = (z ^ (z >>> SIXTEEN)) * ODD_CONST;
		z = z ^ (z >>> SIXTEEN);
		return (z & NON_NEGATIVE) / (float) Integer.MAX_VALUE;
	}
}
